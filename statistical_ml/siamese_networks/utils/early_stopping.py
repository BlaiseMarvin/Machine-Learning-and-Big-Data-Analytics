...
class EarlyStopping:
    def __init__(self, patience=5, min_delta=0.0, checkpoint_path='resnet_cpt.pt', restore_best_weights=True, verbose=True):
        """
            Early stopping - stops training when the validation loss (or metric being tracked) doesn't improve - saves the model + optimizer dicts for resumumption later on
            Args:
             - patience (int) => epochs to wait after last improvement
             - min_delta (float) => minimum change to count as improvment
             - checkpoint_path (str) => file to save the best model to
        ...
        self.best_metric = np.inf # working on an assumption that the validation metric should be reduced
        self.counter = 0
    ...
    def __call__(self, metric, model, optimizer, epoch):
        ...
        if metric < self.best_metric - self.min_delta:
            ...
            self.best_metric = metric # update the metric to beat
            self.counter = 0 # reset the epochs since last improvement counter to zero

            # at this point, save the full checkpoint of => (model + optimizer + epoch)
            torch.save({
                'epoch': epoch,
            ...
            if self.verbose:
                print(f"Metric improved to {metric:.4f}. Checkpoint saved at {epoch}")
        else:
            # no improvement
            self.counter += 1
            if self.verbose:
